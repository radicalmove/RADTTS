...
def test_worker_client_uses_larger_batch_timeout_floor_for_heavy_reference_model(tmp_path: Path):
    client = WorkerClient(
        server_url="http://example.test",
        config_path=tmp_path / "worker.json",
        worker_name="test-worker",
    ...
    payload = WorkerSynthesisEnqueueRequest(
        ...
        pause_config=PauseConfig(seed=1),
        output_format=OutputFormat.WAV,
        output_name="worker-heavy-reference-batch-timeout",
        generate_transcript=False,
        voice_clone_authorized=True,
    )

    timeout = client._batch_timeout_for_request(
        payload,
        "Sentence 2 has several words for a longer reference synthesis run.",
        reference_duration_seconds=28.2,
    )
